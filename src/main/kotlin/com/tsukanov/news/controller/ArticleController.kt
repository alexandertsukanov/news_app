...
import com.tsukanov.news.service.IArticleService
...
@Validated
class ArticleController(private var articleService: IArticleService) {
    @GetMapping("/{id}")
    ...
    fun updateArticle(
        ...
        @Valid @RequestBody requestBody: ArticleDto
    ): ArticleDto = articleService.updateArticle(id, requestBody)

    @PostMapping
    fun saveArticle(
        @Valid @RequestBody requestBody: ArticleDto
    ): ArticleDto = articleService.saveArticle(requestBody)
